...
public:
  using FW_PID = PID<units::AngularSpeed::Dims, units::Voltage::Dims>;
  // Base Units of Angle, controlling angular speed
  using FW_FeedForward = FeedForward<units::Angle::Dims, units::Voltage::Dims>;

  using TBH_Gain_Type
      = units::Quantity<units::Voltage::Dims - units::AngularSpeed::Dims>;
   ...
   */
  Flywheel(motor_group &motors, FW_PID::pid_config_t &pid_config,
   ...
   * @param ratio     ratio of the whatever just multiplies the velocity
   */
  Flywheel(motor_group &motors, FW_FeedForward::ff_config_t &ff_config,
           const double ratio);
   ...
   */
  units::AngularSpeed getDesiredRPM();
  ...
  /**
   * make a measurement of the current RPM of the flywheel motor and return a
   * smoothed version
   */
  units::AngularSpeed measureRPM();

  /**
   * return the current smoothed velocity of the flywheel motors, in RPM
   */
  units::AngularSpeed getRPM();
  /**
  ...
  /**
   * returns the current OUT value of the PID - the value that the PID would
   * set the motors to
   */
  units::Voltage getPIDValue();

  /**
   * returns the current OUT value of the PID - the value that the PID would
   * set the motors to
   */
  units::Voltage getFeedforwardValue();

  /**
   * get the gain used for TBH control
   */
  TBH_Gain_Type getTBHGain();

  /**
   * Sets the value of the PID target
   * @param value - desired value of the PID
   */
  void setPIDTarget(units::AngularSpeed value);

  /**
   * updates the value of the PID
   * @param value - value to update the PID with
   */
  void updatePID(units::AngularSpeed value);
   ...
   */
  void spin_raw(units::Voltage speed, directionType dir = fwd);

  /**
   * Spin motors using voltage; defaults forward at 12 volts
   * FOR USE BY OPCONTROL AND AUTONOMOUS - this only applies if the RPM thread
   * is not running
   * @param speed - speed (between -1 and 1) to set the motor
   * @param dir - direction that the motor moves in; defaults to forward
   */
   ...
   */
  void spinRPM(units::AngularSpeed rpm);
...
private:
  ...
  FW_PID pid;                         // PID on the flywheel
  FW_FeedForward ff;                  // FF constants for the flywheel
  TBH_Gain_Type TBH_gain;             // TBH gain parameter for the flywheel
  double ratio;                       // multiplies the velocity by this value
  std::atomic<units::AngularSpeed> RPM; // Desired RPM of the flywheel.
  task rpmTask; // task (thread but not) that handles spinning the wheel at a
                // given RPM
  FlywheelControlStyle control_style; // how the flywheel should be controlled
  units::AngularSpeed smoothedRPM;
  MovingAverage<units::AngularSpeed::Dims> RPM_avger;
  };
