...
public:
  /**
   * odometry3wheel_cfg_t holds all the specifications for how to calculate
   * position with 3 encoders See the core wiki for what exactly each of
   * these parameters measures
   */
  typedef struct {
    // distance from the center of the left wheel to the center of the right
    // wheel
    units::Length wheelbase_dist;
    /// distance from the center of the robot to the center off axis wheel
    units::Length off_axis_center_dist;
    /// the diameter of the tracking wheel
    units::Length wheel_diam;
   ...
   */
  Odometry3Wheel(CustomEncoder &lside_fwd, CustomEncoder &rside_fwd,
                 CustomEncoder &off_axis, odometry3wheel_cfg_t &cfg,
                 bool is_async = true);

  /**
   * Update the current position of the robot once, using the current state
   * of the encoders and the previous known location
   *
...
private:
  /**
   * Calculation method for the robot's new position using the change in
   * encoders, the old position, and the robot's configuration. This uses a
   * series of arclength formulae for finding distance driven and change in
   * angle. Then vector math is used to combine it with the robot's old position
   * data
   *
   ...
   * @param offax_delta_deg Off-axis (perpendicular) encoder change in rotation,
   * in degrees
   * @param old_pos Robot's old position, for integration
   * @param cfg Data on robot's configuration (wheel diameter, wheelbase,
   * off-axis distance from center)
   * @return The robot's new position (x, y, rot)
   */
  static units::pose_t calculate_new_pos(units::Angle lside_delta_deg,
                                         units::Angle rside_delta_deg,
                                         units::Angle offax_delta_deg,
                                         units::pose_t old_pos,
