...
typename FeedForward<input_dims, output_dims>::ff_config_t
tune_feedforward(vex::motor_group &motor, double pct, double duration)
{
  using namespace unit_literals;
  using FF = FeedForward<input_dims, output_dims>;
  ...
  while (fabs(motor.position(vex::rotationUnits::rev) - start_pos) < 0.05) {
    motor.spin(vex::directionType::fwd, power.Convert(units::volt),
