...
/**
  ...
  */
std::vector<Vector2D::point_config_t> PurePursuit::line_circle_intersections(Vector2D::point_config_t center, double r, Vector2D::point_config_t point1, Vector2D::point_config_t point2)
{
  std::vector<Vector2D::point_config_t> intersections = {};
  ...
  {
    intersections.push_back(Vector2D::point_config_t{.x = x1 + center.x, .y = y1 + center.y});
  }

  if(x2 >= fmin(point1.x, point2.x) && x2 <= fmax(point1.x, point2.x) && y2 >= fmin(point1.y, point2.y) && y2 <= fmax(point1.y, point2.y))
  {
    intersections.push_back(Vector2D::point_config_t{.x = x2 + center.x, .y = y2 + center.y});
  }
 ...
 */
Vector2D::point_config_t PurePursuit::get_lookahead(std::vector<Vector2D::point_config_t> path, Vector2D::point_config_t robot_loc, double radius)
{ 
  //Default: the end of the path
  Vector2D::point_config_t target = path.back();
  ...
  {
    Vector2D::point_config_t start = path[i];
    Vector2D::point_config_t end = path[i+1];

    std::vector<Vector2D::point_config_t> intersections = line_circle_intersections(robot_loc, radius, start, end);
    ...
    for(Vector2D::point_config_t intersection: intersections)
    {
...
*/
std::vector<Vector2D::point_config_t> PurePursuit::inject_path(std::vector<Vector2D::point_config_t> path, double spacing)
{
  std::vector<Vector2D::point_config_t> new_path;
  ...
  for(int i = 0; i < path.size() - 1; i++)
  {
    ...
    Vector2D::point_config_t diff = end - start;
    ...
    for(int j = 0; j < num_points; j++)
    {
      ...
      Vector2D::point_config_t path_point = (Vector2D(start) + vector * j).point();
...
*/
std::vector<Vector2D::point_config_t> PurePursuit::smooth_path(std::vector<Vector2D::point_config_t> path, double weight_data, double weight_smooth, double tolerance)
{
  std::vector<Vector2D::point_config_t> new_path = path;
  ...
  while(change >= tolerance)
  {
    ...
    {
        Vector2D::point_config_t x_i = path[i];
        Vector2D::point_config_t y_i = new_path[i];
        Vector2D::point_config_t y_prev = new_path[i-1];
        Vector2D::point_config_t y_next = new_path[i+1];
        
        Vector2D::point_config_t y_i_saved = y_i;
 ...
 */
std::vector<Vector2D::point_config_t> PurePursuit::smooth_path_hermite(std::vector<hermite_point> path, double steps) {
  std::vector<Vector2D::point_config_t> new_path;
  for(int i = 0; i < path.size() - 1; i++) {
    for(int t = 0; t < steps; t++) {
      // Storing the start and end points and slopes at those points as Vector2Ds.
      Vector2D::point_config_t tmp = path[i].getPoint();
...
std::vector<Vector2D::point_config_t> PurePursuit::smooth_path_cubic(std::vector<Vector2D::point_config_t> path, double res) {
  std::vector<Vector2D::point_config_t> new_path;
