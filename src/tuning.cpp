...
//    else
//    {
//        stored_avg = (stored_avg * (stored_num - 1) / stored_num) + (updateval
//        / stored_num);
//    }
//    return stored_avg;
//}
...
//     }
//     // double avg = (fabs(left_enc.rotation(rev)) +
//     fabs(right_enc.rotation(rev))) / 2.0; double avg =
//     (fabs(left_motors.position(rev)) + fabs(right_motors.position(rev)))
//     / 2.0; if (fabs(avg) < .1)
//     {
//         printf("Diam: 0\n");
//         return;
...
//         right_motors.resetPosition();
//     }
//     // double radius =  ENC_DIFF_IN(left_enc, right_enc) /
//     ((double)times_to_turn * 2 * PI); // radius = arclength / theta double
//     radius = ENC_DIFF_IN(left_motors, right_motors) / ((double)times_to_turn
//     * 2 * PI); // radius = arclength / theta

//    double wheelbase = 2 * radius;

//    main_controller.Screen.clearScreen();
//    main_controller.Screen.setCursor(1, 1);
//    main_controller.Screen.print("Turn the robot in place %d times",
//    times_to_turn); main_controller.Screen.setCursor(2, 1);
...
//    double angle_rad = ENC_DIFF_IN(left_enc, right_enc) /
//    config.dist_between_wheels; double offax_dist = ENC_IN(mid_enc) /
//    angle_rad;
...
//{
//     static timer tmr;
//     static units::Voltage test_pct = 0.0_v;
...
//        if (done || (fabs(left_enc.position(rev)) +
//        fabs(right_enc.position(rev))) > 0)
//        {
//            main_controller.Screen.clearScreen();
//            main_controller.Screen.setCursor(1, 1);
//            main_controller.Screen.print("kS: %f", test_pct);
//            printf("kS: %f\n", test_pct.Convert(units::volt));
//            done = true;
//            return;
//        }
//        else
...
////        units::Speed vel = 0_inps;
////        units::AngularSpeed rot_vel = units::AngularSpeed(0);

////        if (dt == DRIVE)
////        {
////            drive_sys.drive_tank(0.5_v, 0.5_v);
////            vel = odometry_sys.get_speed();
////        }
////        else if (dt == TURN)
////        {
////            drive_sys.drive_tank(0.5_v, -0.5_v);
////            //            rotvel = odometry_sys.get_angular_speed_deg();
////        }

////        units::Quantity<units::Voltage::Dims - units::Length::Dims> kv
////            = (0.5 - ks) / continuous_avg(vel);
...
//    if (main_controller.ButtonA.pressing())
//    {
//        if (dt == DRIVE && (done || drive_sys.drive_to_point(24, 24, fwd,
//        drive_fast_mprofile)))
//        {
//            auto pos = odometry_sys.get_position();
//            printf("%.2f, %.2f, %.2f\n", pos.x, pos.y, pos.rot);
...
//        if (dt == TURN
//            && (done
//                || drive_sys.turn_to_heading(180_deg, *config.turn_feedback)))
//                {
//            auto pos = odometry_sys.get_position();
//            printf("%.2f, %.2f, %.2f\n", pos.x, pos.y, pos.rot);
...
//    else
//    {
//        drive_sys.drive_arcade(main_controller.Axis3.position() / 100.0,
//        main_controller.Axis1.position() / 100.0); drive_sys.reset_auto();
//        done = false;
//    }
//}
...
//        if (dt == DRIVE)
//        {
//            drive_sys.drive_tank(1.0_v, 1.0_v);
...
//{
//     units::Voltage flywheel_target_pct = 0.7_v;
//     static bool new_press = true;
//     static int counter = 0;
//     static MovingAverage<units::AngularSpeed::Dims> avg(20, 0.0_rpm);
//     static MovingAverage<units::AngularSpeed::Dims +
//     units::AngularSpeed::Dims>
//         var(20, 0.0_rpm * 0.0_rpm);
//     if (main_controller.ButtonA.pressing())
//     {
//         counter++;
//         if (new_press)
...
//            = 18.0 * flywheel_motors.velocity(velocityUnits::rpm) * 1_rpm;

//        units::AngularSpeed rpm = rawRPM; // flywheel_sys.getRPM();
//        units::Quantity<units::Voltage::Dims - units::AngularSpeed::Dims> kv
//            = flywheel_target_pct
//              / units::AngularSpeed(continuous_avg(rpm.getValue()));

//        avg.add_entry(rpm);
//        units::AngularSpeed x_bar = avg.get_average();
//        auto squared = (rawRPM - x_bar) * (rawRPM - x_bar);
//        var.add_entry(squared);
//        units::AngularSpeed sd = sqrt(var.get_average());

//        main_controller.Screen.clearScreen();
//        main_controller.Screen.setCursor(1, 1);
//        main_controller.Screen.print("kv: %f", kv);
//        printf("%f %f %f\n", rpm.Convert(units::revolution_per_min),
//               kv.getValue(), sd.Convert(units::revolution_per_min));
//    }
//    else
//    {
//        flywheel_sys.stop();
...
//     static units::AngularSpeed setpt_rpm = 0_rpm;
//     if (first_run)
//     {
//         main_controller.ButtonUp.pressed([]() { setpt_rpm += 500_rpm; });
//         main_controller.ButtonDown.pressed([]() { setpt_rpm -= 500_rpm; });
//         main_controller.ButtonRight.pressed([]() { setpt_rpm += 50_rpm; });
//         main_controller.ButtonLeft.pressed([]() { setpt_rpm -= 50_rpm; });
//     }
...
//        units::AngularSpeed rpm = flywheel_sys.getRPM();
...
//        printf("setpt %f rpm: %f\n",
//               setpt_rpm.Convert(units::revolution_per_min),
//               rpm.Convert(units::revolution_per_min));
//    }
//    else
//    {
...
//     static units::AngularSpeed setpt_rpm = 0_rpm;
//     static double t = 0;
//     static MovingAverage<units::AngularSpeed::Dims> avg_err
//         = MovingAverage<units::AngularSpeed::Dims>(10, 0_rpm);
//     if (first_run)
//     {
//         main_controller.ButtonUp.pressed([]() { setpt_rpm += 500_rpm; });
//         main_controller.ButtonDown.pressed([]() { setpt_rpm -= 500_rpm; });
//         main_controller.ButtonRight.pressed([]() { setpt_rpm += 25_rpm; });
//         main_controller.ButtonLeft.pressed([]() { setpt_rpm -= 25_rpm; });
//         main_controller.ButtonX.pressed([]() { setpt_rpm = 0_rpm; });
...
//     main_controller.Screen.print("setpt: %d", setpt_rpm);
//     avg_err.add_entry(
//         abs(flywheel_sys.getRPM() - flywheel_sys.getDesiredRPM()));
//     printf("%f %f %f %f %f %f \n", t,
//            setpt_rpm.Convert(units::revolution_per_min),
//            flywheel_sys.getRPM().Convert(units::revolution_per_min),
//            (flywheel_sys.getFeedforwardValue() + flywheel_sys.getPIDValue())
//                .Convert(units::volt),
//            flywheel.voltage(volt), flywheel.current(amp));
// }

//// void tune_generic_pid(Feedback &pid2tune, double error_lower_bound, double
//// error_upper_bound)
////{
////     if (pid2tune.get_type() != Feedback::FeedbackType::PIDType)
////     {
////         printf("Cant tune a pid on not a pid? what are you trying to do.
///I'm /         not sad just dissapointed. %s:%d\n", __FILE__, __LINE__);
///return; /     } /     static PID &pid = static_cast<PID &>(pid2tune);
...
////                                         { if (selection == 0){pid.config.p
///+= /                                         sensitivity;} else if (selection
///== /                                         1){pid.config.i += sensitivity;}
///else /                                         {pid.config.d += sensitivity;}
////                                         printf("{\n\tkP: %f\n\tkI:
///%f\n\tkD: /                                         %f\n\tontime:
///%f\n\tdeadband: %f\n}", / pid.config.p, pid.config.i, / pid.config.d, /
///pid.config.on_target_time, / pid.config.deadband); }); /
///main_controller.ButtonDown.pressed([]() / { if (selection == 0){pid.config.p
////                                           -= sensitivity;} else if
///(selection /                                           == 1){pid.config.i -=
///sensitivity;} /                                           else {pid.config.d
///-= /                                           sensitivity;}printf("{\n\tkP:
////                                           %f\n\tkI: %f\n\tkD: %f\n\tontime:
////                                           %f\n\tdeadband: %f\n}",
////                                           pid.config.p, pid.config.i,
////                                           pid.config.d,
////                                           pid.config.on_target_time,
////                                           pid.config.deadband); });
////        main_controller.ButtonR2.pressed([]()
////                                         { if (selection == 0){pid.config.p
///= /                                         0.0 ;} else if (selection == /
///1){pid.config.i = 0.0;} else / {pid.config.d = 0.0;} / printf("{\n\tkP:
///%f\n\tkI: %f\n\tkD: /                                         %f\n\tontime:
///%f\n\tdeadband: /                                         %f\n}\n",
///pid.config.p, pid.config.i, / pid.config.d, / pid.config.on_target_time, /
///pid.config.deadband); }); /        main_controller.ButtonL2.pressed([]() / {
////            pid.config.p = 0.0;
////            pid.config.i = 0.0;
////            pid.config.d = 0.0; });
...
////        func_init = true;
////    }
////    static GraphDrawer output_graph(Brain.Screen, 50, "time", "output",
////    vex::red, true, -1.0, 1.0); static GraphDrawer error_graph(Brain.Screen,
////    50, "time", "error", vex::blue, true, error_lower_bound,
////    error_upper_bound); static double t = 0.0;
...
////    // Draw Graph
////    output_graph.draw(margin, margin, width / 2 - margin, height - 2 *
////    margin); error_graph.draw(margin, margin, width / 2 - margin, height - 2
///* /    margin);
...
// void tune_shooting()
//{
//     static std::atomic<units::AngularSpeed> rpm(0_rpm);
...
//    // Trishot
//    main_controller.ButtonR2.pressed([](){ intake.spin(directionType::fwd,
//    intake_volt, volt); }); main_controller.ButtonR2.released([](){
//    intake.stop(); });

//    // Intake
//    main_controller.ButtonR1.pressed([](){ intake.spin(directionType::rev, 12,
//    volt); }); main_controller.ButtonR1.released([](){ intake.stop(); });

//    VisionAimCommand visaim(false, 0, 20);

//    while(true)
//    {
//        printf("Volt: %1f, Time: %d, rpm: %f, goalX: %d\n",
//        (double)intake_volt,
//               (int)intake_time,
//               rpm.load().Convert(units::revolution_per_sec), 0);
